var VentasMayor = (function ($, win, doc) {
    ...
    var $tblListadoVentasMayor = $('#tblListadoVentasMayor');
    ...
    var $tblTallasVentaMayor = $('#tblTallasVentaMayor');
    ...
    function GetVentasMayor() {
        ...
        var columnDefs = [
            ...
            },
            {
                ...
                'render': function (data, type, full, meta) {
                    if (data == 1) {
                        return "<center>" +
                            '<a class="btn btn-default btn-xs" style="margin-right:0.8em" title="Eliminar" href="javascript:VentasMayor.EliminarVenta(' + meta.row + ')"><i class="fa fa-trash" aria-hidden="true"></i></a>' +
                            '<a class="btn btn-default btn-xs" style="margin-right:0.8em" title="Ver Tallas" href="javascript:VentasMayor.DetalleVenta(' + meta.row + ')"><i class="fa fa-eye" aria-hidden="true"></i></a>' +
                            "</center> ";
                    } else {
                        return "<center>" +
                            '<a class="btn btn-default btn-xs" style="margin-right:0.8em" title="Ver Tallas" href="javascript:VentasMayor.DetalleVenta(' + meta.row + ')"><i class="fa fa-eye" aria-hidden="true"></i></a>' +
        ...
        app.FillDataTableAjaxPaging($tblListadoVentasMayor, url, parms, columns, columnDefs, filters, null, null);
    }
    ...
    function $btnSaveVenta_click() {
        ...
        if (DatosSeleccionadosDetalle.Data.length > 0) {
            ...
            $.each(DatosSeleccionadosDetalle.Data, function (key, value) {
                cant_productos = cant_productos + value.Cantidad
            });

            if (total > 0 && cant_productos >= 6) {
                InsertUpdateVenta();
            } else if (total == 0) {
                app.Message.Info("Aviso", "Ingrese un precio de venta.", null, null);
            } else if (cant_productos < 6) {
                app.Message.Info("Aviso", "Ingrese 6 productos o mas.", null, null);
            }
        }
        else {
    ...
    function InsertUpdateVenta() {
        ...
        DatosSeleccionadosDetalle.Data.map(function (v, i) {
            var objtalla = {
                ...
                "Talla": v.Talla,
                "Cantidad": v.Cantidad,
                "Precio_Prod_Mayor": v.Precio_Prod_Mayor
            }
        ...
        var method = "POST";
        var url = "VentasMayor/InsertUpdateVentasMayor";
        var data = obj;
    ...
    function LoadProductos() {
        ...
        var url = "VentasMayor/GetProducto";
        var parms = {
    ...
    function EliminarVenta(row) {
        var fnAceptarCallback = function () {
            var rowSelect = app.GetValueRowCellOfDataTable($tblListadoVentasMayor, row);
            var obj = {
                "Cod_Venta": rowSelect.Cod_Venta
            }

            var method = "POST";
            var url = "VentasMayor/DeleteVentasMayor";
            var data = obj;
            var fnDoneCallback = function (data) {
    ...
    function GenerarExcel() {
        ...
        var fnDoneCallback = function (data) {
            if (data.InternalStatus === 1 && data.Data.length > 0) {
                app.RedirectTo("VentasMayor/GenerarExcel");
            } else {
                app.Message.Info("Aviso", "No hay ventas con esas fechas", "Aceptar");
            }
        }
        app.CallAjax("POST", "VentasMayor/GetAllVentasMayor", data, fnDoneCallback);
    }
        ...
        var table = $tblListadoProductosSeleccionados.DataTable({
            ...
            columnDefs : [
                ...
                },
                {
                    ...
                    'render': function (data, type, full, meta) {
                        return "<center>" +
                            '<a class="btn btn-default btn-xs"  title="Eliminar" href="javascript:VentasMayor.EliminarProductoSeleccionado(' + meta.row + ')"><i class="fa fa-trash" aria-hidden="true"></i></a>' +
                            "</center> ";
                    }
                }
            ],
    ...
    function LoadTallasSeleccionadas(DatosSeleccionadosDetalle) {
        ...
        var columnDefs = [
            ...
            },
            {
                ...
                'render': function (data, type, full, meta) {
                    return "<center>" +
                        '<a class="btn btn-default btn-xs"  title="Eliminar" href="javascript:VentasMayor.EliminarNroTalla(' + meta.row + ')"><i class="fa fa-trash" aria-hidden="true"></i></a>' +
                        "</center> ";
                }
            }
        ];
    ...
    function ObtenerPrecioVenta() {
        var precio_prod = 0;
        $.each(NuevosDatosSeleccionados, function (key, value) {
            if ($txtModalCodigo.val() == value.Cod_Prod) {
                precio_prod = value.Precio_Prod_Mayor
            }
        });
    ...
    function CalcularPrecioProductos() {
        ...
        $.each(DatosSeleccionadosDetalle.Data, function (key, value) {
            precio_productos = precio_productos + (value.Precio_Prod_Mayor * value.Cantidad)
        });
    ...
    function DetalleVenta(row) {
        ...
        var method = "POST";
        var url = "VentasMayor/TallasVentaMayor";
        var data = obj;
        var fnDoneCallback = function (data) {
            $modalTallas.modal();
            LoadTallaMayor(data);
